...
return function (path, uFlip, vFlip)
    ...
    for line in love.filesystem.lines(path) do
        ...
        elseif firstWord == "f" then
            ...
            for i = 2, #words do
                ...
                if not vt or not uvs[vt] then
                    error("invalid face definition: " .. words[i])
                end
                if not(positions[v][1] and positions[v][2] and positions[v][1]) then
                    error("invalid face definition: " .. words[i])
                end
    ...
    print("59",positions[59][1],positions[59][2],positions[59][3],uvs[59][1],uvs[59][2])
    return result
end
